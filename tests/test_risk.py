...
@pytest.fixture
def correlated_data() -> tuple[torch.Tensor, torch.Tensor]:
...
def test_risk_coverage_basic(
    simple_data: tuple[torch.Tensor, torch.Tensor],
) -> None:
...
def test_risk_coverage_attributes(
    correlated_data: tuple[torch.Tensor, torch.Tensor],
) -> None:
...
def test_plot_basic(simple_data: tuple[torch.Tensor, torch.Tensor]) -> None:
...
def test_plot_selective_curves(
    ...
    fig1 = cast(Figure, rc.plot(reference=False, excess=False))
    assert fig1 is not None
    plt.close(fig1)
    # Test plotting only reference
    fig2 = cast(Figure, rc.plot(empirical=False, excess=False))
    assert fig2 is not None
    plt.close(fig2)
    # Test plotting only excess
    fig3 = cast(Figure, rc.plot(empirical=False, reference=False))
...
def test_plot_importerror(simple_data, monkeypatch):
...
def test_gap_ordering_error(monkeypatch):
    ...
    with pytest.raises(ValueError, match="reference exceeds empirical risk"):
