...
function $g_{\lambda}(x)$ can then be applied at inference time to accept or reject
predictions. We obtain a selective predictor, $h(x)$, that either produces an output 
or abstains from prediction, depending on the score of the input:
...
with the `model` and `loaders` API. This requires the model to expose an `.extract()`
method returning a dictionary with the relevant keys. The example below uses a dummy
model and random data to illustrate the API.
...
(slides are availabe at [https://goergen95.github.io/aoa4dl/docs/slides](https://goergen95.github.io/aoa4dl/docs/slides)). 
...
[![TRR 391 Logo](https://trr391.tu-dortmund.de/storages/trr391/w/bilder/logo/Logo_Transregio_391_RGB.svg){height=64}](https://trr391.tu-dortmund.de/)

This research was funded in the course of [TRR 391 Spatio-temporal Statistics
for the Transition of Energy and Transport](https://trr391.tu-dortmund.de/) (520388526) 
