...
The Deep Latent space Understanding for ConfIdent Decisions (seapig) library
supplies methods for deep learning models to detect low confidence samples and
exclude them from prediction based on the analysis of latent space representations.

The core idea behind the approach is that query
samples that deviate in some form from the embeddings of the
training samples shall be excluded from prediction because
model failure is more likely to occur. 
...
for which we supply an evaluation class which analyses 
the risk-coverage tradeoff of such systems.
