...
# Unified fit() method supports both precomputed embeddings and on-the-fly extraction
...
### Notes and guidance
- **Unified fit() API**: All confidence scores now support a unified `fit()` method that 
  accepts either precomputed embeddings (`X`, `Y`) or a model with loaders 
  (`model`, `loaders`). The deprecated `fit_dl()` method is maintained for backward 
